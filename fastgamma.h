...
#include<math.h> // used for sqrt, sqrtf, pow and powf
#include<stdint.h> // used for uint_fast64_t and uint_fast32_t
...
// Change to 0 if you don't want to use these features! Then,
// the lookup tables won't be used at all, which may be useful
// when the usable memory space is very limited
#define SUBSTITUTE_LOG_FUNCTION 1
#define SUBSTITUTE_POW_FUNCTION 1

#if SUBSTITUTE_LOG_FUNCTION != 0 || SUBSTITUTE_POW_FUNCTION != 0
#if SUBSTITUTE_POW_FUNCTION != 0
const static float P2_ARRAY[256] = {
...
const static float LD_ARRAY[256] = {
 ...
 */
FUNC_INLINE double fast_loggamma(const double x) {
	double_to_i64 ux;
	ux.d = x;
#if SUBSTITUTE_LOG_FUNCTION == 0
 ...
 * checks! This is done intentionally.
 */
FUNC_INLINE float fast_loggammaf(const float x) {
	float_to_i32 ux;
	ux.f = x;
#if SUBSTITUTE_LOG_FUNCTION == 0
	ux.i &= 0x3fffffffUL;
